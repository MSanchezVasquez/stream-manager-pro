...
import { X, Truck, Tv, Mail, Key, Calendar, Compass, Globe } from 'lucide-react';
...
export const SupplierModal: React.FC<SupplierModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (accountToEdit) {
      updatedAccounts = updatedAccounts.map((a) => (a.id === accountToEdit.id ? newAcc : a));
    } else {
      ...
      <div className="w-full max-w-lg rounded-xl bg-white dark:bg-[#141418] border border-slate-200 dark:border-[#1F1F23] shadow-2xl p-6 relative">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="font-bold text-lg text-slate-900 dark:text-[#E4E4E7]">
              {accountToEdit ? 'Editar Cuenta de Proveedor' : 'Añadir Cuenta a Proveedor'}
            </h3>
            <p className="text-xs text-slate-500 dark:text-[#94949E]">
              Proveedor: <strong className="text-purple-600 dark:text-purple-400">{supplier.name}</strong>
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              value={serviceName}
              onChange={(e) => setServiceName(e.target.value as StreamingPlatform)}
